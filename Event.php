...
//        $ip = $_SERVER["REMOTE_ADDR"];
        ...
        if ($business['distribution_rule'] == 'claim') {
            ...
            if ($visiter) {
                ...
                // 替换成最新头像
                $newvisiter = Admins::table('wolive_visiter')->where(['visiter_id' => $visiter_id, 'business_id' => $business_id])->update(['avatar' => $arr['avatar'],'visiter_name' => $arr['visiter_name']]);
                // 最后一次服务的客服id
                ...
                if (!$visiters) {
                    ...
                    $newvisiter = Admins::table('wolive_visiter')->where('visiter_id', $arr['visiter_id'])->update(['state' => 'online','extends'=>$arr['extends'],'visiter_name' => $arr['visiter_name']]);
                }
            ...
            if ($visiter) {
                ...
                Admins::table('wolive_visiter')->where(['visiter_id' => $visiter_id, 'business_id' => $business_id])->update(['avatar' => $arr['avatar'],'login_times'=>Db::raw('login_times+1'),'visiter_name' => $arr['visiter_name']]);
                // 老用户
                ...
                if (!$visiters) {
                    ...
                    $newvisiter = Admins::table('wolive_visiter')->where('visiter_id', $arr['visiter_id'])->update(['state' => 'online','extends'=>$arr['extends']]);
                }
