...
//            ->where('business_id',$_SESSION['Msg']['business_id'])
    ...
    public function addquestion()
    {
        ...
        if (!is_int($status)) {
            $data =['code'=>1,'msg'=>'是否显示字段必须非法'];
            return $data;
        }

        if(isset($post['qid'])){
            $res =Admins::table('wolive_question')->where('qid',$post['qid'])->update(['question'=>$post['question'],'answer'=>$post['answer'],'answer_read'=>$post['answer_read'],'keyword'=>$post['keyword'],'sort'=>$sort,'status'=>$status]);
